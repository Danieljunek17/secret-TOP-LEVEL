    ...
    <div class="box-tools">
        <a href="/server/{{ $server->uuidShort }}/files/add/@if($directory['header'] !== '')?dir={{ $directory['header'] }}@endif">
            <button class="btn btn-success btn-sm btn-icon">
                New File <i class="fa fa-fw fa-file-text"></i>
            </button>
        </a>
        <button class="btn btn-sm btn-success btn-icon" data-action="add-folder">
            New Folder <i class="fa fa-folder-open"></i>
        </button>
        ...
        <div class="btn-group hidden-xs">
            ...
            <ul class="dropdown-menu dropdown-massactions">
                <li><a href="#" id="selective-deletion" data-action="selective-deletion">@lang('server.files.delete') <i class="fas fa-trash-alt"></i></a></li>
            </ul>
        </div>
    </div>
</div>
                ...
                <tr data-type="file" data-mime="{{ $file['mime'] }}">
                    <td class="middle min-size" data-identifier="type"><input type="checkbox" class="select-file hidden-xs" data-action="addSelection">
                        ...
                        @if(in_array($file['mime'], [
                            ...
                            <i class="fa fa-file-archive" style="margin-left: 2px;"></i>
                        @elseif(in_array($file['mime'], [
                            ...
                            <i class="fa fa-file-code" style="margin-left: 2px;"></i>
                        @elseif(starts_with($file['mime'], 'image'))
                            <i class="fa fa-file-image" style="margin-left: 2px;"></i>
                        @elseif(starts_with($file['mime'], 'video'))
                            <i class="fa fa-file-video" style="margin-left: 2px;"></i>
                        @elseif(starts_with($file['mime'], 'video'))
                            <i class="fa fa-file-audio" style="margin-left: 2px;"></i>
                        @elseif(starts_with($file['mime'], 'application/vnd.ms-powerpoint'))
                            <i class="fa fa-file-powerpoint" style="margin-left: 2px;"></i>
                        @elseif(in_array($file['mime'], [
                            'application/vnd.openxmlformatsfficedocument.wordprocessingml.document',
                            'application/vnd.openxmlformatsfficedocument.wordprocessingml.template',
                            'application/msword'
                        ]) || starts_with($file['mime'], 'application/vnd.ms-word'))
                            <i class="fa fa-file-word" style="margin-left: 2px;"></i>
                        @elseif(in_array($file['mime'], [
                            'application/vnd.openxmlformatsfficedocument.spreadsheetml.sheet',
                            'application/vnd.openxmlformatsfficedocument.spreadsheetml.template',
                        ]) || starts_with($file['mime'], 'application/vnd.ms-excel'))
                            <i class="fa fa-file-excel" style="margin-left: 2px;"></i>
                        @elseif($file['mime'] === 'application/pdf')
                            <i class="fa fa-file-pdf" style="margin-left: 2px;"></i>
                        @else
